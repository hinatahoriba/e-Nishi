...
export default function BusinessCard({
  ...
  // テンプレートIDに応じてフロント面コンポーネントを切り替える
  const { component: FrontTemplate } = getTemplateById(templateId);

  // 裏面は全テンプレート共通でstone themeを使用
  const backTheme = CARD_THEMES.stone;
        ...
        >
          ...
          {hasBackContent && (
            ...
            >
              <div className="mt-6">
                <p
                  className={`text-xs tracking-[0.2em] font-medium uppercase ${backTheme.textMuted}`}
                  style={{ fontFamily: "var(--font-cormorant-garamond), serif" }}
                >
                  Connect with me
                </p>
                <div className={`w-4 h-[1px] mx-auto mt-2 ${backTheme.divider}`} />
              </div>
              ...
              {hasPortfolio ? (
                <div className="my-auto flex flex-col items-center space-y-3 z-10">
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className={`group block w-28 h-28 border p-2 rounded transition-transform duration-300 hover:scale-105 ${backTheme.qrBg}`}
                    onClick={(e) => e.stopPropagation()}
                  >
                    <div
                      className={`w-full h-full border rounded flex flex-col items-center justify-center p-1 overflow-hidden ${backTheme.qrInnerBg}`}
                    >
                  ...
                  <p className={`text-[8px] tracking-widest font-sans uppercase ${backTheme.textMuted}`}>
                    Scan or Click
                  </p>
                </div>
              ) : (
                <div className="my-auto flex flex-col items-center space-y-4 opacity-60">
                  <div
                    className={`w-24 h-24 border rounded-2xl flex items-center justify-center shadow-sm ${backTheme.fallbackBg}`}
                  >
                    <div
                      className={`w-20 h-20 border rounded-xl flex items-center justify-center ${backTheme.fallbackInnerBg}`}
                    >
                      <span
                        className={`text-4xl tracking-widest uppercase font-light ${backTheme.textLight}`}
                        style={{ fontFamily: "var(--font-cormorant-garamond), serif" }}
                      >
                  ...
                  <div className="flex flex-col items-center gap-1">
                    <p className={`text-[9px] tracking-[0.3em] font-sans uppercase ${backTheme.textMuted}`}>
                      Digital Card
                    </p>
                    <div className={`w-4 h-[1px] ${backTheme.divider}`} />
                  </div>
                </div>
              )}
              ...
              >
                {(hasPortfolio || hasSNS) && (
                  <div className={`w-6 h-[1px] mx-auto mb-4 ${backTheme.divider}`} />
                )}
